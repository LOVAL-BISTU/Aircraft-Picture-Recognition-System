...
% y=proc(x,fs,snr,i,r) % process clean speed degraded to snr through model i with parameters r
...
%      Version: $Id: psycdigit.m,v 1.10 2011/01/04 15:35:49 dmb Exp $
%
...
while ~quitit
    ...
    if pf(12) || quitit && pf(11)   % 'F/f': plot Psychometric function on figure 101
        ...
        if unimode
            ...
            for jj=1:zmodel
                plot(xax,psychofunc('',[pact(1,jj);m(:,jj,pv(10)) ; pact(2:3,jj); qact(11)],xax));
                hold on
            end
            hold off
        end
